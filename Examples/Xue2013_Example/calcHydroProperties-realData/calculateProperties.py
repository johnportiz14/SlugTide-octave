...
def calc_w(period):
    ''' Calculate angular frequency [rad/timeUnit] given period [timeUnit].'''
...
if __name__== "__main__":
    ...
    prefix = 'data/'
    ampl_data = np.genfromtxt(prefix+'amp_response.csv', delimiter=',',missing_values='NaN')
    eta_data = np.genfromtxt(prefix+'phase_lag.csv', delimiter=',',missing_values='NaN')
    ...
    for i in range(len(ampl_data)):
        ...
        ampl = ampl_data[i,1] #[str/m] or [1/m]
        ...
        #  results = differential_evolution(optimize_SandT, bounds = bnds, args=(ampl, eta, tau, r_c, r_w, b), tol=1.e-10, atol=1e-10, popsize=100)
        #  results = differential_evolution(optimize_SandT, bounds = bnds, args=(ampl, eta, tau, r_c, r_w, b), tol=1.e-25, atol=1e-25, popsize=200)
        #  results = differential_evolution(optimize_SandT, bounds = bnds, args=(ampl, eta, tau, r_c, r_w, b), tol=1.e-25, atol=1e-50, popsize=100)
        #  results = differential_evolution(optimize_SandT, bounds = bnds, args=(ampl, eta, tau, r_c, r_w, b), tol=1.e-100, atol=1e-100, popsize=500)
        #  results = differential_evolution(optimize_SandT, bounds = bnds, args=(ampl, eta, tau, r_c, r_w, b), tol=1e-20, atol=1e-20, popsize=500)
        #  results = differential_evolution(optimize_SandT, bounds = bnds, args=(ampl, eta, tau, r_c, r_w, b), tol=1e-10, atol=1e-10, popsize=100)
        ...
        #  bnds = ( (-6, -1),
                 #  (-8, -1), )
        #  results = minimize(optimize_SandT, x0=(S_de,T_de), args=(ampl, eta, tau, r_c, r_w, b), bounds = bnds, tol=1e-20) #works well, fast
        #  best_S = 10**results.x[0]
        #  best_T = 10**results.x[1]
        #  print('\nFor the parameters:')
        #  print('  Ampl. resp.: {} '.format(ampl))
    ...
    for s in success_array:
        if s: scount+=1
    print('success % = {:.3}%'.format(scount/len(success_array)*100))
