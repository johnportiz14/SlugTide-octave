...
Can run this script interactively on its own:
    python compile_areal_strain.py

or call it with command line arguments (to specify filename identifier):
    python compile_areal_strain.py welldw4
'''
import numpy as np
...
# Read in 0° azimuth file first
d0 = np.genfromtxt('th0_{}'.format(identifier))
# Now read in 90° azimuth file
...
np.savetxt('arealStrains_{}.txt'.format(identifier), areal_strain)
